...
import { ArrowLeft, Play, Loader2, Video } from 'lucide-react';
...
export const YouTubeAnalyzer: React.FC<YouTubeAnalyzerProps> = ({ onBack }) => {
    ...
    const handleRun = async () => {
        ...
        try {
            ...
            addLog("Initializing YouTube Analyzer...", 'info');
            await new Promise(r => setTimeout(r, 600));

            addLog(`Fetching transcript for: ${videoUrl}`, 'tool');
            await new Promise(r => setTimeout(r, 800));

            addLog("Analyzing key insights...", 'analysis');

            const prompt = `Execute the agent task: YouTube Analyzer.\nDescription: Summarize videos and extract key insights.\n\nInputs:\n- Video URL: ${videoUrl}`;
            ...
            <div className="bg-slate-900/50 backdrop-blur-xl border-b border-white/10 sticky top-0 z-20">
                <div className="max-w-4xl mx-auto px-6 py-4">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-xl font-bold">YouTube Analyzer</h1>
                ...
                <div className="grid md:grid-cols-3 gap-8">
                    <div className="md:col-span-2 space-y-8">
                        <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
                            ...
                            <p className="text-gray-400 mb-8">Summarize videos and extract key insights.</p>

                            <div className="space-y-6">
                                <div>
                                    <label className="block text-sm font-medium text-gray-300 mb-2">YouTube Video URL <span className="text-pink-500">*</span></label>
                                    <input
                                        type="text"
                                        placeholder="https://youtube.com/watch?v=..."
                                        className="w-full bg-slate-900 border border-white/10 rounded-lg px-4 py-3 focus:outline-none focus:border-red-500 transition-colors placeholder-gray-600"
                                        value={videoUrl}
                            ...
                            <div className="mt-8 pt-8 border-t border-white/10">
                                <button
                                    onClick={handleRun}
                                    disabled={loading}
                                    className="w-full py-4 bg-gradient-to-r from-red-500 to-pink-600 rounded-xl font-bold text-lg hover:shadow-lg hover:shadow-red-500/20 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                                >
                                    {loading ? (
                                        <>
